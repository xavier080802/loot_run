...
#pragma endregion
		...
		if (teleportCooldown <= 0.f && nextMap) {
			if (!inProceduralMap && map->IsConnector(gPlayer->GetPos())) {
				...
				// Re-init collision grid for procedural map size so collisions continue to work
				{
					AEVec2 nmSize = nextMap->GetFullMapSize();
					GameObjectManager::GetInstance()->InitCollisionGrid(static_cast<unsigned>(nmSize.x), static_cast<unsigned>(nmSize.y));
				}
			}
				...
				// Also ensure grid matches newly generated procedural map
				{
					AEVec2 nmSize = nextMap->GetFullMapSize();
					GameObjectManager::GetInstance()->InitCollisionGrid(static_cast<unsigned>(nmSize.x), static_cast<unsigned>(nmSize.y));
				}
